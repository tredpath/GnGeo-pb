...
void dump_hardware_reg(void) {
    ...
    printf("Sprite Bank:\n");
    for(i=0;i<0x300;i+=2) {
	tctl3 = READ_WORD( &vidram[0x10000 + i] ); /* Zoom */
        tctl1 = READ_WORD( &vidram[0x10400 + i] ); /* Y pos + nb tile */
        tctl2 = READ_WORD( &vidram[0x10800 + i] ); /* X pos */
	if (tctl1&0x40) {
	...
	if (nb_tile>0) {
	    ...
	    for(j=0;j<nb_tile;j++) {
		int tileno,tileatr;
		tileno = READ_WORD(&vidram[offs]);offs+=2;
		tileatr = READ_WORD(&vidram[offs]);offs+=2;
...
static __inline__ Uint16 read_neo_control(void)
{
	...
	return ((scan * 0x80) & 0x7f80)	/* scanline */
	    |(irq_bit << 15)	/* vblank or irq2 */
	    |(conf.pal<<3)
	    |(neogeo_frame_counter & 0x0007);	/* frame counter */
    } else {
	scan = current_line /*+ 22*/;	/* current scanline */
	irq_bit = irq2taken || (scan < 16 || scan > 239);	/* VBLANK or HBLANK */

	return ((scan <<7) & 0x7f80)	/* scanline */
	    |(irq_bit << 15)	/* vblank or irq2 */
	    |(conf.pal<<3)
	    |(neogeo_frame_counter & 0x0007);	/* frame counter */
    }
...
static __inline__ void write_neo_control(Uint16 data)
{
...
static __inline__ void write_irq2pos(Uint32 data)
{
    irq2pos_value = data;
    if (irq2control & 0x20) {
...
{
    //  printf("mem68k_fetch_ram_word %x\n",addr);
    addr &= 0xffff;
...
void update_all_pal(void) {
    int i;
    Uint16 *pc_pal1=(Uint16*)memory.pal_pc1;
    Uint16 *pc_pal2=(Uint16*)memory.pal_pc2;
    for (i=0;i<0x1000;i++) {
...
void mem68k_store_setting_byte(Uint32 addr, Uint8 data)
{
	...
	current_pc_pal = (Uint16 *) memory.pal_pc2;
	return;
    }
