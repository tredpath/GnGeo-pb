/* 
   This file come from mame-0.60, and was modified to work with gngeo
*/
...
the same tables except KOF2000 which uses different ones.
...
/* underlined values are wrong (not enough evidence, FF fill in kof2000) */
const unsigned char kof2000_address_8_15_xor1[256] = {
    0xfc, 0x07, 0x1c, 0x35, 0x72, 0x53, 0xd6, 0x7d, 0x84, 0xa4, 0xc5, 0x93, 0x7b, 0xe7, 0x47, 0xd5,
/*        ^^^^ */
    0x24, 0xa2, 0xfa, 0x19, 0x0c, 0xb1, 0x8c, 0xb9, 0x9d, 0xd8, 0x59, 0x4f, 0x3c, 0xb2, 0x78, 0x4a,
    0x2a, 0x98, 0x9a, 0xf1, 0x1f, 0x22, 0xa8, 0x5b, 0x67, 0xa3, 0x0f, 0x00, 0xfb, 0xdf, 0xeb, 0x0a,
/*  ^^^^^ ^^^^ */
    ...
    0x2e, 0x08, 0x1d, 0xf9, 0xb6, 0x3a, 0x64, 0x96, 0x82, 0x38, 0x9b, 0x9f, 0xa6, 0xde, 0xbc, 0xd9,
/*                                ^^^^        ^^^^              ^^^^  ^^^^  ^^^^  ^^^^ */
};
...
static void decrypt(unsigned char *r0, unsigned char *r1,
		    ...
		    const unsigned char *table1, int base, int invert)
{
...
static void neogeo_gfx_decrypt(int extra_xor)
{
	...
	decrypt(buf + 4 * rpos + 0, buf + 4 * rpos + 3, rom[4 * rpos + 0],
		rom[4 * rpos + 3], type0_t03, type0_t12, type1_t03, rpos,
		(rpos >> 8) & 1);
	decrypt(buf + 4 * rpos + 1, buf + 4 * rpos + 2, rom[4 * rpos + 1],
		rom[4 * rpos + 2], type0_t12, type0_t03, type1_t12, rpos,
		((rpos >> 16) ^ address_16_23_xor2[(rpos >> 8) & 0xff]) &
		1);
	if (rpos%400==0) update_progress_bar(rpos,rom_size >>1);
    }
	...
	if (rpos%400==0) update_progress_bar(rpos+(rom_size >>2),rom_size >>1);
    }
    ...
    {
	int i;
	int tx_size = memory.sfix_size;
	Uint8 *src = memory.gfx + rom_size - tx_size;
	Uint8 *dst = memory.sfix_game;
	for (i = 0; i < tx_size; i++) {
	    dst[i] =
		src[(i & ~0x1f) + ((i & 7) << 2) + ((~i & 8) >> 2) +
		    ((i & 0x10) >> 4)];
	}
    }
    /* readjust the number of tile */
    memory.nb_of_tiles = (rom_size - memory.sfix_size) >> 7;
}
