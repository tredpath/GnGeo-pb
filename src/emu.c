...
#ifdef __QNXNTO__
    snprintf(buf,255,"shared/misc/gngeo/ROMS/shots/%s_%s.bmp", conf.game, date_str);
#else
...
void update_start(void)
{
    ...
    if (conf.nb_joy >= 1)
    {
		if (key[conf.p1_key[BUT_START]] || joy_button[0][conf.p1_joy[BUT_START]])
    ...
    if (conf.nb_joy == 2)
    {
		if (key[conf.p2_key[BUT_START]] || joy_button[1][conf.p2_joy[BUT_START]])
    ...
    { /* Select */
		if (conf.nb_joy >= 1)
		{
			if (key[conf.p1_key[BUT_COIN]] || joy_button[0][conf.p1_joy[BUT_COIN]])
		...
		if (conf.nb_joy == 2) {
			if (key[conf.p2_key[BUT_COIN]] || joy_button[1][conf.p2_joy[BUT_COIN]])
